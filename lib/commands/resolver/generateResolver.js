...
const queryBody = (schemaDefs, orm, userPrimaryKey) =>
  indentEveryLine(concatBlocks([
    ...
    '  },',
    `  async ${schemaDefs.pluralVarName}(root, { _fitter }, { ${schemaDefs.modelName} }) {`,
    `    let allNum=await ${schemaDefs.modelName}.find(_fitter.fitter).count()`,
    `    let acts=${schemaDefs.modelName}.find(_fitter.fitter);`,
    `    if(_fitter.sort){acts=acts.sort(_fitter.sort)}`,
    `    let datas=await acts.limit(_fitter.limit.num).skip(_fitter.limit.index)`,
    `    return {allNum:allNum,page:_fitter.limit.page,pageNum:_fitter.limit.num,data:datas}`,
